...
BEGIN {
    ...
    write-output "BEGIN"
    Write-Output "HW10B2-Enable-User.psl starting"
    ...
    switch ($PSCmdlet.ParameterSetName)
     {
        ...
        $user = Get-ADUser -Filter $pFilter -server $pServer -credential $pCredential
            ...
            foreach ($UserObj in $user){       
                  Enable-ADAccount -Identity $UserObj -Confirm:$pConfirm -Server $pServer -Credential $pCredential 
        ...
        "AccountNameSet"{ 
            ...
            $user = Get-ADUser -Filter {Name -eq $ItemName} -server $pServer -credential $pCredential
                  ...
                  Enable-ADAccount -Identity $user -Confirm:$pConfirm -Server $pServer -Credential $pCredential 
    }   
